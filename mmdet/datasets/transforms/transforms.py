...
class CustomResize(Resize):
    """Resize images & bbox & seg for custom COCO with mask and describtion.
    ...
    def _resize_object_masks(self, results: dict) -> None:
        """Resize object_mask with ``results['scale']``"""
        if results.get('object_mask', None) is not None:
            results['object_mask'] = cv2.resize(results['object_mask'], (results['img_shape'][1], results['img_shape'][0]), interpolation=cv2.INTER_NEAREST)
    ...
    def transform(self, results: dict) -> dict:
        ...
        self._resize_masks(results)
        self._resize_seg(results)
        self._resize_object_masks(results)
        self._record_homography_matrix(results)
        return results
...
@TRANSFORMS.register_module()
class CustomPad(Pad):
    def _pad_object_masks(self, results: dict) -> None:
        """Pad object_mask according to ``self.size``."""
        if results.get('object_mask', None) is not None:
            results['object_mask'] = mmcv.impad(
            results['object_mask'],
            shape=results['pad_shape'],
            pad_val=0,
    ...
    def transform(self, results: dict) -> dict:
        ...
        self._pad_seg(results)
        self._pad_masks(results)
        self._pad_object_masks(results)
        return results
