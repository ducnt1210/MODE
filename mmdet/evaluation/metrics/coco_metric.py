        ...
        for metric in self.metrics:
            ...
            if metric == 'proposal_fast':
                ...
                for i, num in enumerate(self.proposal_nums):
                    ...
                    log_msg.append(f'\nAR@{num}\t{ar[i]:.6f}') # add 2 precision to all metrics
            ...
            if metric == 'proposal':
                ...
                for item in metric_items:
                    val = float(
                        f'{coco_eval.stats[coco_metric_names[item]]:.5f}') # add 2 precision to all metrics
                ...
                if self.classwise:  # Compute per-category AP
                    ...
                    for idx, cat_id in enumerate(self.cat_ids):
                        ...
                        if precision.size:
                            ...
                            ap = float('nan')
                        t.append(f'{nm["name"]}')
                        t.append(f'{round(ap, 5)}') # add 2 precision to all metrics
                        eval_results[f'{nm["name"]}_precision'] = round(ap, 5) # add 2 precision to all metrics
                ...
                for metric_item in metric_items:
                    ...
                    eval_results[key] = float(f'{round(val, 5)}') # add 2 precision to all metrics

                ap = coco_eval.stats[:6]
                logger.info(f'{metric}_mAP_copypaste: {ap[0]:.5f} '
                            f'{ap[1]:.5f} {ap[2]:.5f} {ap[3]:.5f} '
                            f'{ap[4]:.5f} {ap[5]:.5f}') # add 2 precision to all metrics
