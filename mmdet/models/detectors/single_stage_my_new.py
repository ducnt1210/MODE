...
class MyNewSingleStageDetector(BaseDetector):
    ...
    def __init__(self,
        ...
        self.backbone = MODELS.build(backbone)  # 通过注册机制构建
        if neck is not None:
    ...
    def _forward(
        ...
        """
        restored_fea= self.restormer(batch_inputs, embeddings, embedding_masks) # 先通过恢复模块
        x = self.extract_feat(restored_fea) # backbone提取特征，neck融合特征
        results = self.bbox_head.forward(x) # 检测头
        return results
    ...
    def extract_feat(self, batch_inputs: Tensor) -> Tuple[Tensor]:
        ...
        """
        x = self.backbone(batch_inputs) # backbone 提取特征
        if self.with_neck:
            x = self.neck(x)
        return x
