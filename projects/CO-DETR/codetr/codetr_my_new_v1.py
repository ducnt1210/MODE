# Pass language and lanugage mask to the detector for fusing feature after neck layer
import copy
...
from mmdet.models.restormer_language.cpa_arch_v3 import InteractionBlock
...
class FeatEnhancer(nn.Module):
    """
    input:
        x (tuple[Tensor]): Tuple of feature maps from neck. Each feature map
        has shape (bs, dim, H, W).
        l (Tensor): Tensor of language (bs, l_dim, seq_len)
        l_mask (Tensor): Tensor of language mask (bs, seq_len, 1)

    output:
        x (tuple[Tensor]): Tuple of enhanced feature maps. Each has shape (bs, dim, H, W)
    """
    def __init__(self, in_c=256, n_feats=5, l_channels=768, n_depth=1):
        super(FeatEnhancer, self).__init__()
        
        # Create separate InteractionBlock for each feature map
        self.interactions_per_feat = nn.ModuleList([
            nn.ModuleList([InteractionBlock(in_c, l_channels) for _ in range(n_depth)]) 
            for _ in range(n_feats)
        ])

    def forward(self, x, l, l_mask):
        """
        Forward pass through the interaction blocks for each feature map.
        x: tuple of feature maps [(bs, in_c, H, W), ...]
        l: language embeddings (bs, l_dim, seq_len)
        l_mask: language mask (bs, seq_len, 1)
        """
        # Process each feature map with its own interaction block stack
        enhanced_feats = []
        for feat_idx, feat_map in enumerate(x):
            for interaction in self.interactions_per_feat[feat_idx]:
                feat_map = interaction(feat_map, l, l_mask)
            enhanced_feats.append(feat_map)

        return tuple(enhanced_feats)

@MODELS.register_module()
class MyNewCoDETRV1(BaseDetector):
    ...
    def __init__(
            ...
            backbone,
            neck=None,
            feat_enhancer=None, # using text to enhance image feature
        ...
        super(MyNewCoDETRV1, self).__init__(
    ...
    def extract_feat(self, batch_inputs: Tensor, l: Tensor = None, l_mask: Tensor = None) -> Tuple[Tensor]:
        """Extract features.
        ...
        if self.with_neck:
            x = self.neck(x)
        if self.feat_enhancer:
            x = self.feat_enhancer(x, l, l_mask)
        return x
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        if self.feat_enhancer:
            ...
            embedding_masks = embedding_masks.permute(0, 2, 1)
            l2 = embeddings[:, :, max_seq_len_1:]
            l2_mask = embedding_masks[:, max_seq_len_1:, :]
            x = self.extract_feat(restored_fea, l2, l2_mask)
        else:
    ...
    def predict(self,
        ...
        if self.feat_enhancer:
            ...
            l2 = embeddings[:, :, max_seq_len_1:]
            l2_mask = embedding_masks[:, max_seq_len_1:, :]
            img_feats = self.extract_feat(restored_fea, l2, l2_mask)
        else:
