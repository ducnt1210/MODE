...
class PackDetInputs(BaseTransform):
    """Pack the inputs data for the det semantic segmentation /
    panoptic segmentation.ection /
...
class CustomPackDetInputs(PackDetInputs):
    """Pack the custom inputs data for the det semantic segmentation /
    panoptic segmentation.ection /
    ...
    def transform(self, results: dict) -> dict:
        ...
        if 'img' in results:
            ...
            # Refer to https://github.com/open-mmlab/mmdetection/pull/9533
            # for more details
            c_contiguous = img.flags.c_contiguous
            if not img.flags.c_contiguous:
                img = np.ascontiguousarray(img.transpose(2, 0, 1))
                img = to_tensor(img)
            else:
        ...
        if 'object_mask' in results:
            object_mask = results['object_mask']
            ...
            # for more details
            if not c_contiguous:
                object_mask = np.ascontiguousarray(object_mask.transpose(2, 0, 1))
                object_mask = to_tensor(object_mask)
            else:
                object_mask = to_tensor(object_mask).permute(2, 0, 1).contiguous()

            packed_results['inputs']['mask'] = object_mask

        if 'embedding' in results:
            packed_results['inputs']['embedding'] = results['embedding']
        if 'embedding_mask' in results:
            packed_results['inputs']['embedding_mask'] = results['embedding_mask']
        ...
        img_meta = {}
        # print("This is result from CustomPackDetInputs:", results)
        for key in self.meta_keys:
            if key in results:
                img_meta[key] = results[key]
        data_sample.set_metainfo(img_meta)
