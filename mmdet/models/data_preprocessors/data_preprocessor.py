...
@MODELS.register_module()
class DistDetDataPreprocessor(ImgDataPreprocessor):
    """Image pre-processor for detection tasks, but adapt to text and masks.
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        """
        batch_pad_shape = self._get_pad_shape(data)
        data['inputs'] = self.cast_data(data['inputs'])
        temp_data = {
            'inputs': data['inputs']['img'],
            'data_samples': data.setdefault('data_samples', None)
        }

        data['inputs']['mask'] = torch.stack(data['inputs']['mask'])
        data['inputs']['embedding'] = torch.stack(data['inputs']['embedding'])
        data['inputs']['embedding_mask'] = torch.stack(data['inputs']['embedding_mask'])

        temp_data = super().forward(data=temp_data, training=training)
        inputs, data_samples = temp_data['inputs'], temp_data['data_samples']
    ...
    def _get_pad_shape(self, data: dict) -> List[tuple]:
        """Get the pad_shape of each image based on data and
        pad_size_divisor."""
        # print("This is data in val step", data)
